...
- [ ] `cd tools && npm run check` — **`check-panels: OK`**: every catalog row is
      generated by a `WIDGET_SPECS` entry bound to that key, and any row nothing
      serves yet prints as PENDING with the story that owes it. (US-2 left four;
      190 US-3 cleared three of them, so only `quest_journal` remains.)
...
## US-3 (190) — Dialogue + pause/main menu + save/load (Unreal editor required)

**Automated cores are green before the human pass; the UMG screens are
editor-verified (autoMerge off).**

### 0. Automated pre-checks

- [ ] `cd tools && npm run check` — **`check-panels: OK`** and now reports
      **21 of 22** catalog rows generated: `main_menu`, `pause_menu` and
      `save_load` are gated rows rather than PENDING declarations, and only
      `quest_journal` (the export module's own widget) is still deferred.
- [ ] `npm run check:host` — ctest **`ui_chat`**, **`ui_pause_menu`**,
      **`ui_save_slots`** and **`ui_chat_history`** are green (63 assertions,
      13 of them negative controls, including the main menu's Continue gate).
      Before this story `test_dialogue_ui.cpp` was compiled by nothing.

### 1. The screens exist, and the registry resolves them

- [ ] After `GenerateInsimulContent.py`, `/Game/UI/` holds `WBP_MainMenu`
      (parent `InsimulMainMenuPanel`), `WBP_PauseMenu` (`InsimulPauseMenuPanel`)
      and `WBP_SaveLoad` (`InsimulSaveLoadPanel`), each with its bound children.
- [ ] `DA_InsimulUIRegistry` binds `main_menu`, `pause_menu` and `save_load` to
      them, and `UInsimulUIPanelSurface::ResolvePanelClass` returns those classes.
- [ ] Override `pause_menu` in `DA_InsimulUIRegistry` with your own WBP. The ESC
      key opens YOUR shell with no engine code changed — the whole point of the
      registry.

### 2. Dialogue on the streaming SDK (full loop)

- [ ] Talk to an AI character. The player line appears immediately, an empty NPC
      bubble follows with the typing indicator, and response CHUNKS accumulate into
      that one bubble rather than appending new ones.
- [ ] The settled NPC line drives TTS and the `InsimulFaceSync` lip-sync hook —
      the mouth moves to the line that was actually spoken, not to a chunk.
- [ ] Provoke an action trigger (ask for a quest / to trade). The action is
      asserted into the KB and the world reacts; the button/affordance the panel
      offers matches the triggered action's name.
- [ ] Kill the network mid-stream. An `[Error: …]` bubble renders, the turn is
      dropped, and the next `BeginUserTurn` is accepted (no stuck streaming state).
- [ ] Save, quit, reload, re-open the same character: the transcript is there,
      read back out of `save.conversations` — and nothing else in the save moved.

### 3. Pause / ESC menu, module-bundle gated

- [ ] Press Escape in a language-learning world: every gated tab is present.
      In an RPG world the assessment tab is absent, in a strategy world only
      `character` survives among the gated ones.
- [ ] A tab whose hosted PANEL is withheld by the mechanic module set is dropped
      rather than opening an empty box, and `Describe()` / the log names it —
      confirm with a genre that does not select the skill module.
- [ ] Opening the menu lands on the first visible tab; opening straight to a hidden
      tab falls back rather than showing a blank; clicking a tab switches the hosted
      panel and Escape closes.

### 4. Main menu + save/load slots

- [ ] Boot with an empty save directory: **Continue is disabled**, New Game works.
- [ ] With several saves, Continue resumes the most recently saved LOADABLE slot
      (by the codec's `savedAt`, not the file mtime).
- [ ] Tamper with a save file (flip a byte in the payload). Its slot renders as
      **corrupted** with the shared message — "Save file integrity check failed —
      file may be corrupted or tampered." — Load is refused, Save/overwrite is
      still offered, and the row is NOT hidden.
- [ ] With ONLY that corrupted save present, Continue is disabled and the hint text
      says why. It must not offer to resume a save that cannot load.
- [ ] Truncate a save to invalid JSON, and delete another's payload: `invalid_format`
      and `missing_save_file` render as their own messages, not as a generic error.
- [ ] Save into a corrupted slot from the ESC menu's save tab: it recovers, and the
      row turns healthy on the next repaint.

### Deltas vs Unity/Godot

**Target: zero on the three view-model contracts.** The chat turn lifecycle, the
tab-gating matrix and the slot rendering — the corrupted-envelope MESSAGE above
all — are shared corpora (`chat-cases.json`, `pause-menu-cases.json`,
`save-slot-cases.json`); only the widget refs and layout differ by design. A native
leg that words an integrity failure differently, keeps a failed turn in history, or
offers Continue on an unloadable save is a bug against this story.
